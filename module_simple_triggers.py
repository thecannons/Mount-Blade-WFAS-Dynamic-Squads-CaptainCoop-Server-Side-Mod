...
#Notification menus
 ...
 (2, #Error check for multiple parties on the map
	...
	(try_for_parties, ":party_no"),
		...
		(is_between, ":commander", kingdom_heroes_begin, kingdom_heroes_end),
  ...
  (1,
   [
      ...
      (try_begin),
		...
		#(party_set_slot, "$g_player_raiding_village", slot_village_state, svs_normal),
		(party_set_slot, "$g_player_raiding_village", slot_village_raided_by, -1),
      (else_try),
...
#Party AI: pruning some of the prisoners in each center (once a week)
  ...
  (24*15,
   [(try_for_range, ":center_no", centers_begin, centers_end),
	  ...
	  (store_sub, ":difference_bwn_prosperity_and_ideal_prosperity", ":ideal_prosperity", ":prosperity"),
	  (store_div, ":difference_bwn_prosperity_and_ideal_prosperity_div_6", ":difference_bwn_prosperity_and_ideal_prosperity", 6),
	  
      (call_script, "script_change_center_prosperity", ":center_no", ":difference_bwn_prosperity_and_ideal_prosperity_div_6"),
    (try_end),	   
    ]),
  ...
  # Process village raids and reduce prosperity of besieged towns/centers
   (2,
   [
       (call_script, "script_process_village_raids"),

       (call_script, "script_reduce_walled_center_prosperities"),
    ]),
  ...
  # Refresh merchant inventories
   (24,
   [
      (store_mod, ":mod_result", "$g_production_day_count_for_village", 7),
	  (try_begin),
        (eq, ":mod_result", 0),
        ...
        (try_end),
      (try_end),
	  (val_add, "$g_production_day_count_for_village", 1),
    ]),
...
#    ]),
  ...
  # CHANGEFACTION : During rebellion, removing troops from player faction randomly because of low relation points
  # Only lords of player's faction changes faction here.
   (5,
	[
...
##         (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
  ...
  (3,
   [
	...
	(this_or_next|faction_slot_eq, "fac_kingdom_1", slot_faction_leader, "trp_player"),
	(this_or_next|faction_slot_eq, "fac_kingdom_2", slot_faction_leader, "trp_player"),
	(             gt, "$supported_pretender", 0),
	...
	(else_try),
		(faction_get_slot, ":leader", "$players_kingdom", slot_faction_messenger_troop),
	(end_try), 
	(try_begin), 
		(check_quest_active, "qst_oim_getman_radzivill_rebelion"),
		(neg|quest_slot_eq, "qst_oim_getman_radzivill_capture_city", slot_quest_current_state, 0),		
		(eq, "$supported_pretender", "trp_kingdom_1_pretender"),
		(start_map_conversation, "trp_swadian_messenger"),
	(else_try), 	
		(start_map_conversation, ":leader"),
	(end_try), 
	#(eq, "$g_talk_troop", "trp_swadian_messenger"),
    ]),
  ...
  (48, 
   [
       (try_for_range, ":faction_no", kingdoms_begin, kingdoms_end),
	     (faction_slot_eq, ":faction_no", slot_faction_respawning_frequency, 0),
		 (store_random_in_range, ":random_frequency", 0, 80),
		 (val_add, ":random_frequency", 20), #20..100, average is 60
	     (faction_set_slot, ":faction_no", slot_faction_respawning_frequency, ":random_frequency"),
       (try_end),
       ...
       (try_for_range, ":troop_no", kingdom_heroes_begin, kingdom_heroes_end),
         ...
         (try_begin),
           ...
           (try_begin),
		     (faction_get_slot, ":random_frequency", ":cur_faction", slot_faction_respawning_frequency),
			 (store_random_in_range, ":random_value", 0, ":random_frequency"),
			 (lt, ":random_value", 10),

             (call_script, "script_create_kingdom_hero_party", ":troop_no", ":center_no", 1),
             (party_attach_to_party, "$pout_party", ":center_no"),
		   (try_end),
         (else_try),
		   #CHANGEFACTION : lords who cannot spawn because there is no available center, can change faction by time here.
       ...
       (try_for_parties, ":party_no"),
         ...
         (try_begin),
           ...
           (try_begin),
             (store_random_in_range, ":random_no", 0, 100),
             (lt, ":random_no", 5),
             (call_script, "script_do_party_center_trade", ":party_no", ":home_center", 4),
         ...
         (else_try),
           (try_begin),
             ...
             (call_script, "script_do_party_center_trade", ":party_no", ":cur_ai_object", 4),
             (assign, ":total_change", reg0),
			 ...
			 #note : prosperity increase of village with 70% (increased it was 35%) during each trading
             (try_begin),
               (store_random_in_range, ":rand", 0, 100),
               (lt, ":rand", 70),			   
...
##    ]),
  ...
  # Consuming food at every 6 hours
  (6,
   [
     (call_script, "script_cf_food_consumption"), 
    ]),
  ...
  (0,
   [(troop_get_inventory_slot, ":cur_horse", "trp_player", 8), #horse slot
	...
	(assign, ":new_icon", "icon_oim_voz"),
    (else_try),
	(try_begin),
           (eq, "$g_player_icon_state", pis_normal),
           (try_begin),
             (ge, ":cur_horse", 0),
             (assign, ":new_icon", "icon_player_lord"),
           (else_try),
 ...
 #Update how good a target player is for bandits
  (2,
   [
       (try_for_parties, ":party_no"),
	     (party_get_ignore_with_player_party, ":ignore_with_player_party", ":party_no"),
		 
		 (try_begin),
		   (ge, ":ignore_with_player_party", 0),
		   (val_sub, ":ignore_with_player_party", 2),
		   (val_max, ":ignore_with_player_party", 0),
		   (party_set_ignore_with_player_party, ":party_no", ":ignore_with_player_party"),

		   (str_store_party_name, s1, ":party_no"),
		   (assign, reg1, ":ignore_with_player_party"),		   
	     (try_end),

		 (try_begin),
		   (party_slot_eq, ":party_no", slot_party_type, spt_kingdom_caravan),
		   (party_set_bandit_attraction, ":party_no", 50),
		 (try_end),

	     (party_stack_get_troop_id, ":commander", ":party_no", 0),
		 
		 (this_or_next|is_between, ":commander", heroes_begin, heroes_end),
         (eq, ":commander", "trp_player"),
         
		 (store_troop_gold, ":total_value", ":commander"),

         (troop_get_inventory_capacity, ":inv_size", ":commander"),
         (try_for_range, ":i_slot", 0, ":inv_size"),
           (troop_get_inventory_slot, ":item_id", ":commander", ":i_slot"),
           (ge, ":item_id", 0),
           (try_begin),
         ...
         (party_set_bandit_attraction, ":party_no", ":bandit_attraction"),
	   (try_end),
    ]),
  ...
  (1,
   [(neg|map_free),
    ...
    (try_begin),
	  ...
	  (val_add, "$oim_illness_recovery_status", 10),
	(try_end),
  ...
  (24, [(try_for_range, ":cur_quest", all_quests_begin, all_quests_end),
          (try_begin),
            ...
            (try_begin),
              ...
              (val_sub, ":exp_days", 1),
              (try_begin),
					(eq, ":cur_quest", "qst_oim_sem_samuraev"), 
					(eq, ":exp_days", 0),
					(call_script, "script_fail_quest", ":cur_quest"),
				(else_try),
					(eq, ":cur_quest", "qst_oim_lendliz2"), 
					(eq, ":exp_days", 0),
					(call_script, "script_fail_quest", ":cur_quest"),
				(else_try),
					(eq, ":cur_quest", "qst_oim_kidalovo_z_konyem"), 
					(eq, ":exp_days", 0),
					(call_script, "script_change_player_honor", -2), 
					(quest_get_slot, ":quest_trp",  ":cur_quest", slot_quest_giver_troop), 
					(call_script, "script_change_player_relation_with_troop", ":quest_trp", -15),
					(call_script, "script_fail_quest", ":cur_quest"),				
					(call_script, "script_end_quest", ":cur_quest"),
				(else_try),
					(eq, ":cur_quest", "qst_oim_invest"), 
					(eq, ":exp_days", 0),
					(call_script, "script_change_player_honor", -2), 
					(quest_get_slot, ":quest_trp",  ":cur_quest", slot_quest_giver_troop), 
					(call_script, "script_change_player_relation_with_troop", ":quest_trp", -15),
					...
					(call_script, "script_end_quest", ":cur_quest"),
				(else_try),
					(eq, ":cur_quest", "qst_oim_invest_2"), 
					(eq, ":exp_days", 0),
					(call_script, "script_change_player_honor", -3), 
					(quest_get_slot, ":quest_trp",  ":cur_quest", slot_quest_giver_troop), 
					(call_script, "script_change_player_relation_with_troop", ":quest_trp", -30),
					(call_script, "script_change_player_relation_with_faction", "fac_kingdom_5", -15),
					(call_script, "script_fail_quest", ":cur_quest"),				
					(call_script, "script_end_quest", ":cur_quest"),
				(else_try),
					(eq, ":cur_quest", "qst_oim_monfor_shved"), 
					(eq, ":exp_days", 0),
					(quest_get_slot, ":quest_trp",  ":cur_quest", slot_quest_giver_troop), 
					(call_script, "script_change_player_relation_with_troop", ":quest_trp", -15),
					(call_script, "script_fail_quest", ":cur_quest"),
					(call_script, "script_end_quest", ":cur_quest"),
					(call_script, "script_fail_quest", "qst_oim_monfor_shved_army"),
				...
				(else_try),
					(eq, ":cur_quest", "qst_oim_duel_wound"),
					(eq, ":exp_days", 0), 
					(troop_raise_attribute, "trp_player", ca_strength, 1), 
					(troop_raise_attribute, "trp_player", ca_agility, 2), 
					(call_script, "script_end_quest", "qst_oim_duel_wound"),
				(else_try),
					(eq, ":cur_quest", "qst_oim_dmitriy_elections"),
					(eq, ":exp_days", 0), 
					(assign, ":all_count", 0),
					(assign, ":supporters_count", 0),
					(try_for_range, ":troop_no", kingdom_heroes_begin, kingdom_heroes_end), 
						(store_troop_faction, ":faction", ":troop_no"),
						(eq, ":faction", "fac_kingdom_2"),
						(val_add, ":all_count", 1), 
						(troop_get_slot, ":hero_support", ":troop_no", slot_troop_support_hero),
						(eq, ":hero_support", 1),
						(val_add, ":supporters_count", 1), 
					(end_try), 
					(val_mul, ":all_count", 50),
					(val_div, ":all_count", 100),
					(try_begin),
						(ge, ":supporters_count", ":all_count"),
						(jump_to_menu, "mnu_oim_dmitry_elected"), 
					(else_try), 
						(jump_to_menu, "mnu_oim_dmitry_election_failed"), 
					(end_try), 
				(else_try), 
					(is_between, ":cur_quest", "qst_oim_illness_1", "qst_oim_potop_capture_city"),
					(eq, ":exp_days", 0),
					(troop_raise_attribute, "trp_player", ca_strength, 1), 
					(troop_raise_attribute, "trp_player", ca_agility, 1), 
					(quest_set_slot, ":cur_quest", slot_quest_expiration_days, -1),					
...
(4, 
   [(try_for_range, ":troop_no", kingdom_heroes_begin, kingdom_heroes_end),
	...
	(24,
	[
		(try_for_range, ":troop_no", oim_tavern_visitors_begin, oim_tavern_visitors_end), 
			(troop_set_slot, ":troop_no", slot_troop_alcohol_count, 0),
			(troop_set_slot, ":troop_no", slot_troop_simple_fights_count, 0),
			(troop_set_slot, ":troop_no", slot_troop_last_duel_time, -1),
       	(try_end), 
		(assign, "$g_oim_hero_alcohol_count", 0),
	]),
	
	(24*7, 
	[
		#slightly decreases global alcohol count for the player per week
		(try_begin),
			(neg|check_quest_active, "qst_oim_alcoholic"), 
			(val_mul, "$g_oim_hero_global_alc_count", 90),
			(val_div, "$g_oim_hero_global_alc_count", 100),
		(else_try),
			(quest_slot_eq, "qst_oim_alcoholic", slot_quest_current_state, 0),
			(val_mul, "$g_oim_hero_global_alc_count", 80),
			(val_div, "$g_oim_hero_global_alc_count", 100),
		(end_try), 
	]), 
	...
	(24, 
	[  
		(check_quest_active, "qst_oim_alcoholic"), 
		(store_troop_gold, ":gold", "trp_player"), 
		#(val_mul, ":gold", 2), 
		(val_div, ":gold", 100), 
		...
		(try_begin), 
			(quest_get_slot, ":start_amount", "qst_oim_alcoholic", slot_quest_target_amount), 
			(val_add, ":start_amount", 10), 
			(le, ":start_amount", "$g_oim_hero_global_alc_count"), 
			(quest_set_slot, "qst_oim_alcoholic", slot_quest_current_state, 1), 
		(end_try), 
	]), 
    ...
    # Adding troops to the castle for taverna
    (72,
    [
    ...
    (1,
    [
	  (try_begin), #after one illness finishes about 2 days no illness should occur to save player.
	    (eq, "$oim_illness_count", 0),
	    (gt, "$oim_illness_recovery_status", 0),
		(val_sub, "$oim_illness_recovery_status", 3), #300 / 3 = about 100 hours about 4 days he will not face with any new diseases.
		(val_max, "$oim_illness_recovery_status", 0),
	  (try_end),

	  (call_script, "script_recalculate_ills_count"), 
	  (try_begin),
	    (call_script, "script_check_food_variation"),
		(try_begin),
		  (eq, reg0, 0),
		  (eq, "$oim_illness_count", 0),
		  (eq, "$oim_illness_recovery_status", 0),
		  (call_script, "script_start_quests_illness"),
		  (assign, "$oim_illness_recovery_status", 0),
		(try_end),
	  (try_end),
	  ...
	  (try_begin),
		...
		(try_for_range, ":unused", 0, "$oim_illness_count"),

		  (try_begin), #fever (high effecting)
		    (eq, ":cur_illness", 0),
		    (assign, ":max_random", 15),
		  (else_try), #gout (low effecting)
		    (eq, ":cur_illness", 1), 
		    (assign, ":max_random", 20),
		  (else_try), #dropsy (moderate effecting)
		    (eq, ":cur_illness", 2), 
		    (assign, ":max_random", 15),
		  (else_try), #dysentery (moderate effecting)
		    (eq, ":cur_illness", 3),
		    (assign, ":max_random", 15),
		  (else_try), #scabies (high effecting)
		    (assign, ":max_random", 10),
		  (try_end),

		  (store_random_in_range, ":random", 0, ":max_random"),
		  (eq, ":random", 0),

		  (call_script, "script_illness_day_action"),
		(try_end),
	  (try_end),
    ]),
    ...
    [
	  ...
	  (try_begin),
	    (gt, "$oim_illness_count", 0),
		(try_begin),
		  (ge, "$oim_illness_recovery_status", 300),
		  (call_script, "script_end_one_illness_quest"),
		(try_end),
		
		(try_begin),
		  (eq,"$g_camp_mode", 1),
		  (call_script, "script_doctor_check"),
		  (assign, ":has_doctor", reg0),
		  (try_begin),
		    (eq, ":has_doctor", 1),
			(eq, "$oim_illness_camp_start", 1),
			(store_current_hours,"$oim_illness_camp_start"),
		  (else_try),
		    (eq, ":has_doctor", 1),
			(neg|eq, "$oim_illness_camp_start", 0),
			(store_current_hours,":cur_time"),
			(store_sub, ":cur_time_interval", ":cur_time", "$oim_illness_camp_start"),
			(try_begin),
			  (ge, ":cur_time_interval", 24),
			  (call_script, "script_end_one_illness_quest"),
			(try_end),
		  (try_end),
		(try_end),
  ...
  (24*6,
   [
	(call_script, "script_check_faction_count"),
	(try_begin),
		(gt, reg0, 0),
  ...
  (24,
   [
    (try_for_range, ":cur_faction", kingdoms_begin, kingdoms_end),
		(neg|faction_slot_eq, ":cur_faction", slot_faction_state, sfs_defeated),
		(neg|faction_slot_eq, ":cur_faction", slot_faction_state, sfs_inactive),
		(try_for_range, ":cur_template", "pt_forager_party", "pt_messenger_party"),
			(call_script, "script_count_parties_of_faction_and_party_template", ":cur_faction", ":cur_template"),
			(assign, ":cur_count", reg0),
			(try_begin),
				(lt, ":cur_count", 7),
				(call_script, "script_spawn_party", ":cur_faction", ":cur_template"),
			(try_end),
		(try_end),
	(try_end),
	...
	(try_begin), 
		...
		(quest_slot_eq, "qst_oim_potop_zagloba_revoult", slot_quest_current_state, 1),
        (call_script, "script_get_information_about_troops_position", "trp_oim_zagloba_qst", 1),
		(try_begin),
			(neq, reg0, 0),
			(troop_get_type, reg1, "trp_oim_zagloba_qst"),
			(str_store_string, s2, "@The last time you heard about {reg1?her:him}, {s1}"),
			#(display_log_message, "@The last time you heard about {reg1?her:him}, {s1}"),
			(add_quest_note_from_sreg, "qst_oim_potop_marshal", 4, s2, 1),
		(end_try), 
	(end_try), 
	
    ]),

  # Check attack condition and 5 in limit
  (0.1,
   [
    (call_script, "script_check_attack_and_dist_condition"),
    ]),
  ...
  [
    (try_for_parties, ":party_no"),
		...
		(try_begin), 
			...
			(try_begin), 
				(neg|is_between, ":party_faction", "fac_player_supporters_faction", "fac_kingdoms_end"),
				(call_script, "script_cf_oim_remove_party", ":party_no"),
			(end_try), 
		(end_try), 
		#0 party size bug fix
		(try_begin),
			(gt, ":party_no", "p_reserved_3"),
     ...
     (try_begin),
       ...
       (try_for_range_backwards, ":i_stack", 0, ":num_stacks"),
	     ...
	     (try_begin),
           ...
           (assign, ":start_deserting_morale_limit", 44),
         (else_try),
           (eq, ":reduce_campaign_ai", 1), #moderate
           (assign, ":start_deserting_morale_limit", 38),
         (else_try),                        
           (eq, ":reduce_campaign_ai", 2), #easy 
           (assign, ":start_deserting_morale_limit", 32),
         (try_end),  

         (lt, ":troop_morale", ":start_deserting_morale_limit"),
		 (store_add, ":start_deserting_morale_limit_plus_4", ":start_deserting_morale_limit", 4),
         (store_sub, ":desert_prob", ":start_deserting_morale_limit_plus_4", ":troop_morale"),
